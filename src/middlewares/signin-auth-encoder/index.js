...
module.exports = async (req, res, next) => {
    ...
    try {
        if (username && password) {
            const account_id = await Accounts.log(username, password);
            Thread.onFindOne(Informations, { _id_account: account_id }, {
                ref1: '_id_account',
                ref2: '_id_config'
            }).then(user => {
                ...
                if (secondary_auth_token) {
                    ...
                    Log.show(`/POST/signin-authentication-encoder SUCCESS: ${username} has been login`);
                }
            }).catch(err => {
                throw new Error(err);
            })
        }
        else { throw new Error('Username and Password is incorrect'); }
    }
    catch (err) {
